...
import 'package:anaaj/screens/volunteer/providers/current_assigned_order_stream_provider.dart';
...
class VolunteerDashboardScreen extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: Builder(
        builder: (context) {
          if (appUser != null) {
            Volunteer volunteer = appUser.appUser as Volunteer;
            final currentOrderStream =
                ref.watch(currentAssignedStreamProvider(volunteer.id));
            return currentOrderStream.when(
              data: (data) {
                if (data.docs.isEmpty) {
                  ...
                  try {
                    final rawOrder = data.docs.firstWhere(
                      (element) =>
                          element.data()['orderStatus'] !=
                          OrderStatus.delivered.name,
                    );
                    final parsedOrder = Order.fromJson(rawOrder.data());
                    return OrderPreviewScreen(order: parsedOrder);
                  } catch (e) {
                    print(e);
                    return Column(
                      children: [
                                  ...
                                  IconButton(
                                    onPressed: () async {
                                      final Uri openDialer = Uri(
                                        scheme: 'tel',
                                        path: "+91" +
                                            order.donorInstituition.phoneNumber
                                                .toString(),
                                      );
                                  ...
                                  IconButton(
                                    onPressed: () async {
                                      final Uri emailLaunchUri = Uri(
                                        scheme: 'mailto',
                                        path: order
                                            .receiverInstituition.emailAddress,
                                      );
                                      await launchUrl(emailLaunchUri);
                                    },
                                    icon: Icon(Icons.mail_outline_rounded),
